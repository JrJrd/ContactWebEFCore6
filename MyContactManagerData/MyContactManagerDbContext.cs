...
namespace MyContactManagerData
{
    ...
    {
        private static IConfigurationRoot _configuration; /*The _configuration variable is declared as a static field of type IConfigurationRoot.
                                                          This variable will be used to access configuration settings, including the connection string.*/
        public DbSet<State> States { get; set; }
        public DbSet<Contact> Contacts { get; set; }
        public MyContactManagerDbContext()  //scaffold
        {
            //To manually create an instance of the MyContactManagerDbContext class without specifying any specific options.
        }
        //leverage connection strings from appsettings
        public MyContactManagerDbContext(DbContextOptions options) : base(options) /*By passing the options parameter to the base class constructor,
                                                                                   it allows the base class to configure the DbContext with the provided options.*/
        {
        ...
        {                                                                      /*to bring in connection string
                                                                                The OnConfiguring method is overridden from the DbContext base class.
                                                                                This method is called internally by Entity Framework Core when configuring 
                                                                                the DbContext with options. */
            var builder = new ConfigurationBuilder()
                .SetBasePath(Directory.GetCurrentDirectory())
                .AddJsonFile("appsettings.json", optional: true, reloadOnChange: true); //optional, if not found, it wont return an error
            _configuration = builder.Build();
            var cnstr = _configuration.GetConnectionString("MyContactManager"); //connection string
            optionsBuilder.UseSqlServer(cnstr);
            /*The SetBasePath method is called on the ConfigurationBuilder to set the base path for configuration files. 
         Directory.GetCurrentDirectory() returns the current directory where the application is executing.
         This builder is responsible for loading and accessing configuration settings.*/
        }
